...
A minimal Telegram bot that forwards each message to a pluggable LLM connector and replies with the result. There's no Telegram SDK: the Bot API is called directly over HTTPS with Node's built-in `fetch`. Every message is handled independently; no conversation history is kept.
