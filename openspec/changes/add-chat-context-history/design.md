...
- Provide `/reset-context` to clear one chat's history on demand.
...
- No editing/deleting individual turns — only whole-chat clear via `/reset-context`.
...
- **[Risk]** Unbounded history (per explicit user decision) can eventually make the prompt sent to the LLM very large for long-running chats, degrading latency/quality or exceeding the provider's context window. → **Mitigation**: none in this change (explicitly out of scope by decision); `/reset-context` is the user's escape hatch. A future change can add trimming if this becomes a real problem.
...
- Rollout is a normal deploy: no flag needed since `/reset-context` is opt-in per chat and the "one-shot" behavior for a chat's very first message is unchanged (empty history). If needed, an operator can delete `data/history.db` to fully reset (equivalent to every chat running `/reset-context`).
