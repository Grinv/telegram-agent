...
Three reductions are taken, and each removes wording rather than capability:
...
*The instruction text stops repeating the tool definitions* (~30 estimated tokens). "inside the sandbox" appears in each of seven tool descriptions and again in the instruction; the instruction is the wrong place for it.
